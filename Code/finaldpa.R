...
library(ROCR)
pr <- prediction(x_predict,test$is_diabetic)
prf <- performance(pr,measure = "tpr",final.measure = "fpr")
plot(prf)
...
library('e1071')
naive_bayes_model <- naiveBayes(as.factor(is_diabetic)~., data = train[-2])
nb_predict <- predict(naive_bayes_model, newdata = test[-c(2,9)],type = "class")
View(nb_predict)
...
# SVM model
#ncol(x)
svm_model <- svm(is_diabetic~.,data = train[-2])
svm_predict <- predict(svm_model,newdata = test[-c(2,9)])
...
norm_train <- as.data.frame(lapply(train[-c(1,2,3,5,6,7)], normalize))
norm_test <- as.data.frame(lapply(test[-c(1,2,3,5,6,7)], normalize))
#View(norm_test)
dim(norm_train)
dim(norm_test)
...
final2 <- final %>% mutate_if(is.character, as.factor)
str(final2)
rf <- randomForest(as.factor(is_diabetic)~., ntree=100, data = train[-2], importance=TRUE)
rf
varImpPlot(rf,sort = T,n.var = 31, main = "Important Variables")
predict_rf <- predict(rf,test[-c(2,9)])
View(predict_rf)
...
nn = neuralnet( f, train_new, hidden = 3, linear.output = FALSE, threshold = 0.5 ) # Accuracy : 0.8700968   


#summary(nn)
plot( nn, rep = "best" )
#test<-test[-c(1,40)]
predictNN <- neuralnet::compute(nn, test_new[,-c(2,5)])
...
predict_gbm <- predict(object = gbm_model,newdata = test[-9],n.trees = 1500,type = "response")
...
View(test)
pr_gbm <- prediction(predict_gbm,test[,9])
...
conf_matrix_gbm1 <- table(default.pred_gbm,test[,9])
...
set.seed(1234)
dtree_fit <- train(as.factor(is_diabetic)~., data = train[-2], method = "rpart", parms = list(split = "information"), trControl = trctrl, tuneLength = 10)
dtree_fit
prp(dtree_fit$finalModel, box.palette = "Reds", tweak = 1.2)
cart_predict <- predict(dtree_fit, newdata = test[-c(2,9)])
