...
shiny::shinyApp(
  ...
  server = function(input, output, session) {
    ...
    processdata<-function(){
      ...
      ## Taking input from Shiny form
      input <- read.table('C:/Users/chintana/OneDrive/Masters/Projects/DPA/Code/responses/input.csv',sep=",", header=TRUE)
      head(input)
      inputPredProb <- predict(modelfinalGLM,input,type = "response")
      inputPred <- ifelse(inputPredProb > 0.5,"Oops! Better cut down on the Sugar !! Looks like you are Diabetic :-(","Congrats !!Treat yourself to an IceCream , You are not Diabetic :-)")
      inputPred<-inputPred[2]
      ###
      inputPred
          }
    ...
    observeEvent(input$submit, {
      ...
      print("before displayin")
      output$result <- renderPrint({inputPred})
